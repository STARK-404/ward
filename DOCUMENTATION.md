...
- **Donor Registration**: Citizens can register as blood donors with their blood group
- **Availability Toggle**: Donors can mark themselves as available/unavailable
- **Blood Group Matching**: Uses compatibility chart to find matching donors
- **Emergency Notifications**: Push notifications sent to all matching donors in the ward
- **Compatible blood groups considered**: Universal donor (O-) can donate to all
...
### Blood Donation
```
POST   /api/blood-donation/register      # Register as blood donor
PUT    /api/blood-donation/availability  # Update availability
GET    /api/blood-donation/my-status     # Get donor status
GET    /api/blood-donation/donors        # List donors in ward
POST   /api/blood-donation/request       # Request blood (sends notifications)
PUT    /api/blood-donation/complete      # Mark donation complete
```
